        ...
        if (schoolName) {
            ...
            if (scheduleData) {
                ...
                if (startDate && (todayStr < startDate || todayStr > endDate)) {
                    ...
                    throw new Error(`Jadwal ujian untuk sekolah Anda (Gelombang ${(scheduleData.gelombang || '-').replace('Gelombang ', '')}) adalah tanggal ${dateRangeStr}. Anda tidak dapat login di luar jadwal tersebut.`);
                }
            }
        }
    }
